...
ob_re_var <- list(ret_obj, var_obj)
ob_re_var
```

```{r Looked at the weights}
re_wts_tan <-c(na.omit(extractWeights(re_tan_port)))
```

The __Minmimum Variance Portfolio__ minimizing the variance of the portfolio
...
wts_tan <- c(extractWeights(object = maxSR.lo.ROI))
wts_minvar <- c(extractWeights(object =opt_minvar))
```
...
bench_returns
```
```{r Rebalanced Returns Tangency Portfolio}
tangency_returns <- PerformanceAnalytics::Return.portfolio(R=stock_returns_xts,
                                       wealth.index = FALSE,
                                       geometric = TRUE,
                                       rebalance_on = "months",
                                       contribution = FALSE,
                                       maxSR=TRUE)
tangency_returns
dates <- bench_returns$date
tangency_returns <- rbind(tangency_returns$portfolio.returns)
```

```{r Rebalanced Returns Minimum Variance Portfolio}
minvar_returns <- PerformanceAnalytics::Return.portfolio(R=stock_returns_xts,
                                                         weights=wts_minvar,
                                                         wealth.index= FALSE,
                                                         geometric=TRUE,
                                                         rebalance_on= "months",
...
tan <- as.data.frame(tangency_returns)
minvar <- as.data.frame(minvar_returns)
ov_tan_port <- cbind(tan,bench_returns)
ov_minvar_port <- cbind(minvar,bench_returns)
RF <- 0
ov_tan_port
ov_minvar_port
```
...
cap_tan <-ov_tan_port %>%
    tq_performance(Ra = portfolio.returns, Rb = Benchmark, 
    performance_fun = table.CAPM)
cap_tan
```

```{r Overview for the Minimum Variance}
cap_minvar <- ov_minvar_port %>%
    tq_performance(Ra = portfolio.returns, Rb = Benchmark, 
    performance_fun = table.CAPM)
cap_minvar
```
...
```{r Sharpe Ratio}
sharp_tan <- (cap_tan$AnnualizedAlpha-RF)/sd(ov_tan_port$portfolio.returns)
sharp_minvar <- (cap_minvar$AnnualizedAlpha-RF)/sd(ov_minvar_port$portfolio.returns)
...
```{r Future Value}
library(readxl)
return_calculation <- read_excel("return_calculation.xlsx")
FV_tan <- return_calculation$cum_tan
FV_minvar <- return_calculation$cum_minvar
FV_bench <- return_calculation$cum_bench
id <- c(1:251)
ov_list <- cbind(id, FV_tan,FV_minvar,FV_bench)
ov_list <- dplyr::as_data_frame(ov_list)
invest <- ov_list[251,]
invest
```

As we see the tangency portfolio, would have the highest return compared to the MinVariance and the benchmark portfolio.
...
PV_tan <- return_calculation$tan_one
PV_minvar <- return_calculation$minvar_one
PV_bench <- return_calculation$bench_one
PV_tan <- as.data.frame(PV_tan)
PV_minvar <- as.data.frame(PV_minvar)
PV_bench <- as.data.frame(PV_bench)
id <- c(1:nrow(PV_tan))
id <- as.data.frame(id)
ov_list <- cbind(id, PV_tan,PV_minvar,PV_bench)
ov_list <- as.data.frame(ov_list)
invest <- ov_list[nrow(ov_list),]
invest
```
As we see if we would like to make an one time investment, but also the exact same amount of money as if we would invest 100 dollar per month. For the Tangency Portfolio we would need to invest __11055,75__$ for the Minimium Variance Portfolio we would need to invest __16048.26__$ and for the passive strategy S&P500 we would need __25494.39__. So, as we can see that the interest of interest effect takes here part and dramatically increases the future value of our investment.
...
For the same two portfolios and the appropriate benchmark calculate overall performance measures (Sharpe ratio, M2 [assume a risk-free rate of 0], Treynor Ratio, Jensen's Alpha and Information ratio). Interpret. Additional do the two market timing regressions (ch 24.4) and see whether your portfolios can "time" the market.
...
MarketTiming(Ra = ts_ov_tan_port[,1],Rb = ts_ov_tan_port[,3],Rf = 0,method = "TM")
MarketTiming(Ra = ts_ov_minvar_port[,1],Rb = ts_ov_minvar_port[,13],Rf = 0,method = "TM")
```
