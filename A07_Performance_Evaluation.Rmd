...
tangency_returns <- PerformanceAnalytics::Return.portfolio(R=stock_returns_xts,
                                       weights = extractWeights(maxSR.lo.ROI),
                                       wealth.index = FALSE,
                                       geometric = TRUE,
                                       rebalance_on = "months",
                                       contribution = TRUE)
...
try <- 100*(1+tangency_returns$portfolio.returns)
dicount <- try*((1+tangency_returns$portfolio.returns) ^ nrow(tangency_returns))
sum(dicount)
```
...
```{r Future Value}
FV_tan <- fv(r = tan$portfolio.returns, n = 1:251, pv = 0,pmt = -try,type = 0)
FV_minvar <- FV(rate = minvar$portfolio.returns, nper = 1:251, pv = 100, pmt = -100, type=0)
FV_bench <- FV(rate=bench_returns$Benchmark, nper = 1:251, pv = 100, pmt = -100, type = 0)
id <- c(1:251)
...
PV_tan <- PV(rate = tan$portfolio.returns,nper = 1:239,fv =-invest$FV_tan ,pmt = 0,type = 1)
PV_minvar <- PV(rate =minvar$portfolio.returns, nper=1:239,fv=-invest$FV_minvar,pmt=0,type=1)
PV_bench <- PV(rate=bench_returns$Benchmark, nper=1:239, fv=-invest$FV_bench,pmt=0, type=1)
id <- c(1:239)
