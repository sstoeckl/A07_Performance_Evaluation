...
```{r}
# Among us, we have defined which person does which task. Unfortunately one person did not stick to the plan. Therefore, we could not deal with the content and theory because we did not receive the R-code at the agreed time. 
```
...
pacman::p_load(timetk,PortfolioAnalytics,tidyverse,tidyquant,FFdownload,PortfolioAnalytics,nloptr,psych)
```
...
stock_returns <-tq_get(x = stock_names,
                       get  = "stock.prices", 
                       from = "2000-01-01", 
                       to   = "2020-11-18") %>%
  group_by(symbol) %>%
